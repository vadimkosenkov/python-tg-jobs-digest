...
An automated Python script that aggregates job postings from selected Telegram channels, filters them by specific frontend keywords (Angular, React, etc.), removes duplicates, and sends a clean daily digest directly to your Telegram **"Saved Messages"** (me).
...
* **Secure:** Your personal phone number, API keys, and session data are fully encrypted using GitHub Secrets and never exposed in the public repository.
...
Before deploying to GitHub Actions, you must perform a one-time interactive login to generate your Telegram session file.
...
* Once successful, it will generate `job_digest_session.session` and `job_digest_state.json` files locally.
...
The workflow is pre-configured to run automatically every day at **09:00 UTC** (12:00 MSK/Minsk). You can also trigger it manually anytime using the **"Run workflow"** button in the Actions tab.
